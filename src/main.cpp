...
// ---------- Profiles ----------
// PROF_P90_BRIDGE,  PROF_HUMBUCKER_BRIDGE are missing
...
void applyProfile(ProfileId id) {
  ...
  if (g_currentPps == 0) g_currentPps = min<uint32_t>(g_targetPps, 500);
...
void startWinding() {
  ...
  g_donePulses = 0;
  g_targetPps = rpmToPps(g_targetRPM);
  if (g_targetPps < 10) g_targetPps = 10;
  if (g_currentPps == 0) g_currentPps = min<uint32_t>(g_targetPps, 500);
  if (g_currentPps < 10) g_currentPps = 10;
...
void handleHmiCmd(const String& s) {
  ...
  if (s.startsWith("UI:PROFILE:IDX:")) {
    ...
    if (idx >= 0 && idx < PROF_COUNT) selectProfileId((uint8_t)idx);
    else nxStatusErr("profile idx OOR");
    return;
  }
...
void handleBusLine(const String& line) {
  ...
  if (payload.startsWith("PG:")) {
    ...
    if (mv < 4600) nxStatusErr("fault:PG_LOW");
    else nxStatusOK(buf);
    return;
  }
...
void loop() {
  ...
  while (HMI.available()) {
    ...
    if (c == '\n' || c == '\r') {
      hmiLine.trim();
      while (hmiLine.length() && (uint8_t)hmiLine[0] <= 0x20) hmiLine.remove(0, 1);
      if (hmiLine.startsWith("UI:")) handleHmiCmd(hmiLine);
      hmiLine = "";
    } else if (hmiLine.length() < 120) hmiLine += c;
  }
  ...
  while (BUS.available()) {
    ...
    if (c == '\n' || c == '\r') {
      busLine.trim();
      if (busLine.length()) handleBusLine(busLine);
      busLine = "";
    } else if (busLine.length() < 180) busLine += c;
  }
  ...
  if (millis() - rampMs >= 10) {
    ...
    if (delta > dead) g_currentPps += (uint32_t)min<int32_t>(delta, step);
    else if (delta < -dead) g_currentPps -= (uint32_t)min<int32_t>(-delta, step);
    else g_currentPps = g_targetPps;  // snap to target inside deadband
  ...
  if (g_state == SOFT_STOP) {
    ...
    if (g_currentPps <= 20) {  // near-zero threshold
      g_state = IDLE;
      nxSetStatus("stopped");
    }
  }
